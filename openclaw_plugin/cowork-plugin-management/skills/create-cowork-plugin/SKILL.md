...
- `openclaw.plugin.json/plugin.json` is always required
- Component directories (`skills/`, `agents/`) go at the plugin root, not inside `openclaw.plugin.json/`
- Only create directories for components the plugin actually uses
- Use kebab-case for all directory and file names

### plugin.json Manifest

Located at `openclaw.plugin.json/plugin.json`. Minimal required field is `name`.
...
This schema is shared with Claude Code's plugin system, but you're creating a plugin for OpenClaw, a desktop app for doing knowledge work.
...
3. Run `claude plugin validate <path-to-plugin-json>` to check the plugin structure. If this command is unavailable (e.g., when running inside Cowork), verify the structure manually:
   - `openclaw.plugin.json/plugin.json` exists and contains valid JSON with at least a `name` field
